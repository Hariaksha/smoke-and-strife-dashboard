"""Weekly incremental update for the Nigeria wind-IV panel, mirroring
pipeline/run_update.py's structure and per-source try/except isolation
(one source's failure can't block the others or stop results_nigeria.json
from being written with whatever vintages are actually available).

Does NOT re-run the placebo test or Conley SEs - those are point-in-time
validation exercises (mirroring how Indonesia's own paper treats them as
appendix-level checks, not part of the live weekly re-estimation).
Their results are baked into nigeria/data/robustness.json with a
checked_at timestamp, refreshed only when nigeria/analysis/placebo_test.py
or conley_se.py are re-run manually.

Run:  python -m nigeria.pipeline_update        (from the repo root)
"""
import json
...
def update_conflict(meta, districts):
    acled = acled_mod.fetch_acled_country('Nigeria', '2015-01-01')
