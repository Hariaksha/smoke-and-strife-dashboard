"""Nigeria FIRMS fetch, mirroring pipeline/fetch_fires.py exactly (same
SP/NRT split, same preliminary-flagging logic) but with:
  - Nigeria's bounding box instead of Indonesia's
  - MAX_DAYS=5, not 10 - pipeline/fetch_fires.py's AREA/MAX_DAYS are frozen
    module-level constants computed at import time, so monkey-patching
    config.BBOX (which works for fetch_wind.py, which re-reads config.BBOX
    fresh on every call) would NOT affect it. Kept as a separate module
    rather than patched at runtime.
"""
import io
import time
...
MAX_DAYS = 5  # verified empirically - see pipeline/fetch_fires.py's docstring
